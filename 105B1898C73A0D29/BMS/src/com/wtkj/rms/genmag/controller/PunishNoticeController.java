package com.wtkj.rms.genmag.controller;
...
@Controller
@RequestMapping({ "/punishNotice" })
public class PunishNoticeController extends BaseProcessController {
	// 流程名称
	...
	@Autowired
	private PunishNoticeServiceI punishNoticeService;

	@RequestMapping({ "/manager" })
	public String manager(HttpServletRequest request) {
		return "/basic/punishNotice/punishNotice";
	}

	@RequestMapping({ "/dataGrid" })
	@ResponseBody
	public Grid dataGrid(HttpServletRequest request,
			PunishNoticeVo punishNotice, PageFilter ph) {
		Grid grid = new Grid();
		User user = getLoginUser(request);
		List<PunishNotice> pos = punishNoticeService.dataGrid(user,
				punishNotice, ph);
	

		List<PunishNoticeVo> pos1 = new ArrayList<PunishNoticeVo>();
		/*for (punishNotice pn : pos) {
			pos1.add(toVo(pn));
		}*/
		grid.setRows(pos);
		grid.setTotal(punishNoticeService.count(user, punishNotice, ph));
		return grid;
	}
	...
	public String viewPage(HttpServletRequest request, String viewType, Long id) {
		...
		PunishNotice punishNotice = null;
		if (!StringUtils.isEmpty(id) && Long.valueOf(id) > 0) {
			// 编辑，详情，处理
			//carRentalReg = carRentalRegService.get(id);
			punishNotice = punishNoticeService.get(id);
		} else {
			// 添加
			punishNotice = new PunishNotice();
		}

		request.setAttribute("punishNotice", (punishNotice));
		request.setAttribute("viewType", viewType);
		// 申请人和申请时间

		// 根据类型返回 缴纳或者退回页面
		return "/basic/punishNotice/punishNoticeView";
	}
	 ...
	 * @return
	 */
	private PunishNoticeVo toVo(PunishNotice po) {
		PunishNoticeVo vo = new PunishNoticeVo();
